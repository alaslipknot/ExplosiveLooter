...
namespace Animancer
{
    ...
    public abstract partial class MixerState : AnimancerState
    {
        ...
        /// </summary>
        /// <remarks>
        /// If any of the states are being synchronised (according to <see cref="SynchroniseChildren"/>), only those
        /// states will be included in the getter calculation.
        /// </remarks>
        ...
        {
            var synchronisedStates = ObjectPool.AcquireList<AnimancerState>();
            GatherSynchronisedChildren(synchronisedStates);

            totalWeight = 0;
            normalizedTime = 0;
            length = 0;

            for (int i = synchronisedStates.Count - 1; i >= 0; i--)
            {
                var state = synchronisedStates[i];
            ...
            ObjectPool.Release(synchronisedStates);
        ...
        /// <summary>Adds all synchronised children (according to <see cref="SynchroniseChildren"/>) to the list.</summary>
        public void GatherSynchronisedChildren(List<AnimancerState> synchronisedChildren)
        {
            ...
            var flagCount = _SynchroniseChildren == null ? 0 : Mathf.Min(_SynchroniseChildren.Length, childCount);

            // Gather states flagged as true.
            for (int i = 0; i < flagCount; i++)
            {
                ...
                synchronisedChildren.Add(state);
            }

            // Gather all states that don't have flags.
        ...
        {
            var states = ObjectPool.AcquireList<AnimancerState>();
            GatherSynchronisedChildren(states);
            var count = states.Count;
            if (count <= 1)
            {
                ObjectPool.Release(states);
                return;
            }
        ...
        public override string ToString()
        {
            ...
            if (prefixLength > 0)
                mixerName.Append(' ').Append(childNames[0], 0, prefixLength);
        ...
        public abstract class TransitionDrawer : ManualMixerState.Transition.Drawer
        {
            /************************************************************************************************************************/
            ...
            protected abstract void AddThresholdFunctionsToMenu(GenericMenu menu);
