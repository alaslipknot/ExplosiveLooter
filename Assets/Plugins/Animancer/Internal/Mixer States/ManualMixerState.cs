    ...
    public class ManualMixerState : MixerState
    {
        ...
        public virtual void Initialise(int childCount)
        {
            ...
            if (childCount <= 1 && OptionalWarning.MixerMinChildren.IsEnabled())
                OptionalWarning.MixerMinChildren.Log($"{GetType()} is being initialised with {nameof(childCount)} <= 1." +
                ...
                if (_Speeds != null)
                {
                    ...
                    if (_Speeds.Length != 0 && _Speeds.Length != _Clips.Length)
                        Debug.LogError(
                            ...
                            $" does not match the number of clips ({_Clips.Length}).", State.Root?.Component as Object);
#endif
        ...
        public class Transition : Transition<ManualMixerState>
        {
            ...
            public class Drawer : Editor.TransitionDrawer
            {
                ...
                /// </summary>
                protected virtual void GatherSubProperties(SerializedProperty property) => GatherSubPropertiesStatic(property);
