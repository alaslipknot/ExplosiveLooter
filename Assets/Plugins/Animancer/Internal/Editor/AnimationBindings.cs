    ...
    public sealed class AnimationBindings : AssetPostprocessor
    {
            ...
            public void LogIssues(AnimancerState state, MatchType match)
            {
                ...
                switch (match)
                {
                    ...
                    case MatchType.Unknown:
                        message.Append("The animation bindings have not yet been checked.");
        ...
        public static void AssertIcons()
        {
            ...
            for (int i = 0; i < matchTypes.Length; i++)
            {
                ...
                {
                    case MatchType.Correct:
                        Debug.Assert(icon == null, $"The icon for {nameof(AnimationBindings)}.{match} should be null.");
                        break;

                    case MatchType.Unknown:
                        for (int iIcon = 0; iIcon < Icons.Unknown.Length; iIcon++)
                            Debug.Assert(Icons.Unknown[iIcon] != null,
                                $"The icon for {nameof(AnimationBindings)}.{nameof(MatchType.Unknown)}[{iIcon}] was not loaded.");
                        break;
                    ...
                    case MatchType.Error:
                    default:
                        Debug.Assert(icon != null, $"The icon for {nameof(AnimationBindings)}.{match} was not loaded.");
                        break;
                }
            }
        }
